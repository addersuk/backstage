...
import {
  getRubygemsFileFetchUrl,
  getRubygemsIntegrationRelativePath,
  getRubygemsRequestOptions,
  RubygemsIntegration,
  ScmIntegrations,
 ...
 * @public
 */
export class RubygemsUrlReader implements UrlReader {
  static factory: ReaderFactory = ({ config, treeResponseFactory }) => {
    const integrations = ScmIntegrations.fromConfig(config);
    return integrations.rubygems.list().map(integration => {
      const reader = new RubygemsUrlReader(integration, {
        treeResponseFactory,
      });
  ...
  constructor(
    private readonly integration: RubygemsIntegration,
  ...
  async readUrl(
    ...
    try {
      response = await fetch(builtUrl, {
        headers: {
          ...getRubygemsRequestOptions(this.integration.config).headers,
  ...
  async readTree(
    ...
    const relativePath = getRubygemsIntegrationRelativePath(
    ...
    const projectRubygemsResponse = await fetch(
      new URL(
        `${this.integration.config.apiBaseUrl}/projects/${encodeURIComponent(
          repoFullName,
        )}`,
      ).toString(),
      getRubygemsRequestOptions(this.integration.config),
    );
    if (!projectRubygemsResponse.ok) {
      const msg = `Failed to read tree from ${url}, ${projectRubygemsResponse.status} ${projectRubygemsResponse.statusText}`;
      if (projectRubygemsResponse.status === 404) {
        throw new NotFoundError(msg);
      }
      throw new Error(msg);
    }
    const projectRubygemsResponseJson = await projectRubygemsResponse.json();

    // ref is an empty string if no branch is set in provided url to readTree.
    const branch = ref || projectRubygemsResponseJson.default_branch;
    ...
    const commitsRubygemsResponse = await fetch(
      new URL(
        `${this.integration.config.apiBaseUrl}/projects/${encodeURIComponent(
          repoFullName,
    ...
    if (!commitsRubygemsResponse.ok) {
      const message = `Failed to read tree (branch) from ${url}, ${commitsRubygemsResponse.status} ${commitsRubygemsResponse.statusText}`;
      if (commitsRubygemsResponse.status === 404) {
        throw new NotFoundError(message);
      }
      throw new Error(message);
    }

    const commitSha = (await commitsRubygemsResponse.json())[0]?.id ?? '';
  ...
  private async getGitlabFetchUrl(target: string): Promise<string> {
    ...
    if (targetUrl.pathname.includes('/-/jobs/artifacts/')) {
      return this.getRubygemsArtifactFetchUrl(targetUrl).then(value =>
        value.toString(),
      );
    }
    // Default to the old behavior of assuming the url is for a file
    return getRubygemsFileFetchUrl(target, this.integration.config);
  }
  ...
  private async getRubygemsArtifactFetchUrl(target: URL): Promise<URL> {
